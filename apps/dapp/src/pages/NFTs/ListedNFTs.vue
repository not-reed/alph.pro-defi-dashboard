...
const sortBy = ref<SortBy>(SortBy.Volume)
...
const nfts = computed(() => nftResults.value.map(nft => ({
    ...
    stats: {
        ...
        floor: Number(nft.stats.floor) / 1e18 * alphPrice.value,
        volume: Number(nft.stats.volume) / 1e18 * alphPrice.value
    }
})).sort((a, b) => {
    switch (sortBy.value) {
        case SortBy.Name:
        ...
        case SortBy.HolderCount:
            return Number(b.stats.holderCount) - Number(a.stats.holderCount);
        case SortBy.ListedCount:
            return Number(b.stats.listedCount) - Number(a.stats.listedCount);
        case SortBy.MintedCount:
            return Number(b.stats.mintedCount) - Number(a.stats.mintedCount);
        case SortBy.Floor:
            return Number(b.stats.floor) - Number(a.stats.floor);
        case SortBy.Volume:
            return Number(b.stats.volume) - Number(a.stats.volume);
    }
}))
</script>
    ...
    <div>
        <div>
            <div class="flex gap-4 justify-between max-w-4xl pt-4 mx-4">
                <div class="text-lg font-bold">Listed NFTs</div>

                <div class="flex items-center gap-2">
                    <div>Sort by:</div>
                    <select v-model="sortBy" class="bg-zinc-300 dark:bg-calypso-900 p-2">
                ...
                <div class="flex flex-col gap-4 justify-between h-full">
                    ...
                    <div class="flex flex-col gap-4">
                        <ul>
                            ...
                            <li><span class="opacity-75">Floor:</span> {{ format(nft.stats.floor) }}</li>
                            <li><span class="opacity-75">Volume:</span> {{ format(nft.stats.volume) }}</li>
                        </ul>
