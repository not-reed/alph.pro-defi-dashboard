...
const pricedTokens = computed(() => user.tokens.reduce((acc, balance) => {
...
}, [] as PricedToken[]).sort((a: PricedToken, b: PricedToken) => {
            ...
            <li v-for="balance in pricedTokens"
                ...
                <div class="flex gap-2 justify-end w-full col-span-3 pb-2 md:pb-0" v-if="true">
                    <component :is="balance.token.social?.twitter ? 'a' : 'span'" :href="balance.token.social?.twitter"
                        target="_blank" :class="balance.token.social?.twitter ? '' : 'opacity-25 cursor-not-allowed'"
                    ...
                    <component :is="balance.token.social?.github ? 'a' : 'span'" :href="balance.token.social?.github"
                        target="_blank" :class="balance.token.social?.github ? '' : 'opacity-25 cursor-not-allowed'"
                        class="h-4 w-4 md:h-6 md:w-6 text-calypso-500 dark:text-calypso-50">
                    ...
                    <component :is="balance.token.social?.website ? 'a' : 'span'" :href="balance.token.social?.website"
                        target="_blank" :class="balance.token.social?.website ? '' : 'opacity-25 cursor-not-allowed'"
                        class="h-4 w-4 md:h-6 md:w-6 text-calypso-500 dark:text-calypso-50">
                    ...
                    <component :is="balance.token.social?.discord ? 'a' : 'span'" :href="balance.token.social?.discord"
                        target="_blank" :class="balance.token.social?.discord ? '' : 'opacity-25 cursor-not-allowed'"
                        class="h-4 w-4 md:h-6 md:w-6 text-calypso-500 dark:text-calypso-50">
                        <Discord class="h-4 w-4 md:h-6 md:w-6 text-calypso-500" />
                    </component>

                    <component :is="balance.token.social?.telegram ? 'a' : 'span'"
                        :href="balance.token.social?.telegram" target="_blank"
                        :class="balance.token.social?.telegram ? '' : 'opacity-25 cursor-not-allowed'"
                        class="h-4 w-4 md:h-6 md:w-6 text-calypso-500 dark:text-calypso-50">
