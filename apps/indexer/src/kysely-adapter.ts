/**
 * Copied from @auth/kysely-adapter
 * however the original does not cooperate with current database structure
 * so this was copied and modified as needed
 */
	...
	return {
		...
		async updateUser({ id, ...user }: UserUpdate) {
			...
			const result = supportsReturning
				...
				: query.executeTakeFirstOrThrow().then(() =>
						db
							.selectFrom("User")
							.selectAll()
							.where("id", "=", id as UserId)
