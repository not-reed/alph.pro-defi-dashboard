...
function probablyDeadRareState(state: NodeState) {
	if (state.immFields.length !== 2) {
		return false;
	}

	if (state.mutFields.length !== 8) {
		return false;
	}

	const immFieldTypes = ["U256", "ByteVec"];
	for (const type in immFieldTypes) {
		if (state.immFields[type].type !== immFieldTypes[type]) {
			return false;
		}
	}
	...
	for (const type in mutFieldTypes) {
		if (state.mutFields[type].type !== mutFieldTypes[type]) {
			return false;
		}
	}
...
async function fetchPaginatedSubContracts(collection: ContractAddress) {
	const subContractsUrl = `${config.EXPLORER_URL}/contracts/${collection}/sub-contracts`;
	...
	do {
		const subContractResponse = await fetchOrCallback(
			`${subContractsUrl}?limit=${limit}&page=${page}`,
			CACHE_TIME,
			async () =>
				await fetch(`${subContractsUrl}?limit=${limit}&page=${page}`, {
...
export async function fixCollection(collection: ContractAddress) {
	const stateUrl = `${config.NODE_URL}/contracts/${collection}/state`;
	console.log("Fetching State");
	const state = await fetchOrCacheJson(stateUrl);
	const probablyDeadRare = probablyDeadRareState(state);
	if (!probablyDeadRare) {
		console.log("seems not a deadrare collection, aborting");
		return;
	}

	console.log("fetching subcontracts");
	const subcontracts = await fetchPaginatedSubContracts(collection);

	const totalSupply = Number(state.mutFields[4].value);
