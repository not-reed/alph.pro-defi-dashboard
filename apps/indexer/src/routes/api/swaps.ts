...
app.get("/liquidity", async (c) => {
	const swaps = await db
		...
		.where("AyinLiquidityEvent.pairAddress", "not in", [
			"uVYr7bR2P4oG5ZZHN9dodEk1Fk2czxaD1eFoiv6REcas", // gold castle lottery
			"wFuF4KgTCXRmpjn7panP2Mimk7B1MLkYXeGDTNy8ahsu", // alphpro subscriptions
		])
		.orderBy("timestamp desc")
		.limit(25)
		.execute();
