...
async function fetchCoingeckoPrice(): Promise<any> {
  return await fetch(
    "https://api.coingecko.com/api/v3/simple/price?ids=alephium,ayin,bitcoin,ethereum,tether,usd-coin&vs_currencies=usd"
  ).then((a) => a.json());
}
...
export async function updateCoinGeckoPrices() {
  ...
  coingeckoPrices.usdc = Number(resp["usd-coin"].usd);
}
...
export async function saveCoingeckoPrices() {
  savePrices([
    ...
    {
      address: "22Nb9JajRpAh9A2fWNgoKt867PA6zNyi541rtoraDfKXV", // usdc
      price: BigInt(coingeckoPrices.usdc * 1e18),
      liquidity: null, // used to determine weight of price source
      source: "coingecko", // coingecko | ayin
      sourceKey: "usd-coin", // pairAddress, coingecko id, etc
      timestamp: new Date(),
    },
  ]);
}
